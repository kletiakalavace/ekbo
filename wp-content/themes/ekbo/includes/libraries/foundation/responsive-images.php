...
function dhwpfstart_custom_sizes( $sizes ) {
	return array_merge( $sizes, array(
		'fp-small'  => __( 'FP Small', 'dhwpfstart' ),
		'fp-medium' => __( 'FP Medium', 'dhwpfstart' ),
		'fp-large'  => __( 'FP Large', 'dhwpfstart' ),
		'fp-xlarge'  => __( 'FP XLarge', 'dhwpfstart' )
	) );
}
add_filter( 'image_size_names_choose', 'dhwpfstart_custom_sizes' );

// Add custom image sizes attribute to enhance responsive image functionality for content images
function dhwpfstart_adjust_image_sizes_attr( $sizes, $size ) {
...
add_filter( 'wp_calculate_image_sizes', 'dhwpfstart_adjust_image_sizes_attr', 10 , 2 );

// Remove inline width and height attributes for post thumbnails
function dhwpfstart_remove_thumbnail_dimensions( $html, $post_id, $post_image_id ) {
    if(!strpos($html, 'attachment-shop_single')) {
        $html = preg_replace( '/^(width|height)=\"\d*\"\s/', '', $html );
    }
    return $html;
}
add_filter( 'post_thumbnail_html', 'dhwpfstart_remove_thumbnail_dimensions', 10, 3 );
