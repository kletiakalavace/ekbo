 ...
 */
if ( ! function_exists( 'dhwpfstart_top_bar_r' ) ) {
	function dhwpfstart_top_bar_r() {
		wp_nav_menu( array(
			'container'      => false,
 ...
 */
if ( ! function_exists( 'dhwpfstart_mobile_nav' ) ) {
	function dhwpfstart_mobile_nav() {
		wp_nav_menu( array(
			'container'      => false,                         // Remove nav container
			'menu'           => __( 'mobile-nav', 'dhwpfstart' ),
...
if ( ! function_exists( 'foundationpress_add_menuclass' ) ) {
	function dhwpfstart_add_menuclass( $ulclass ) {
		$find = array('/<a rel="button"/', '/<a title=".*?" rel="button"/');
		$replace = array('<a rel="button" class="button"', '<a rel="button" class="button"');

		return preg_replace( $find, $replace, $ulclass, 1 );
	}
	add_filter( 'wp_nav_menu','dhwpfstart_add_menuclass' );
}
...
if ( ! function_exists( 'dhwpfstart_breadcrumb' ) ) {
	function dhwpfstart_breadcrumb( $showhome = true, $separatorclass = false ) {
		...
		if ( ! is_front_page() ) {
			...
			if ( is_single() && ! is_attachment() ) {
				...
				// Paginated archives
				echo '<li class="current item-current-' . get_query_var('paged') . '">' . __('Page', 'dhwpfstart' ) . ' ' . get_query_var('paged') . '</li>';
